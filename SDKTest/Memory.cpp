...
Memory::Memory()
{
	puts("Initializing memory class...");
...
{
	//should not happen! Only if you didnt call the class constructor!
	...
	if (status == initialized)
	{
		...
		if (!baseAddress) {
			puts("Error getting base address!");
			return noBaseAddress;
		}

		if (!processID) {
			puts("Error getting process ID!");
			return noProcessID;
		}

		puts("Loaded Memory class!");
	}
...
void Memory::checkStatus()
{
	if (status != loaded) {
#ifdef NDEBUG
		throw std::runtime_error("status is not loaded! Did you forget to initialize the memory class or the target game?");
#else
		DebugBreak();
#endif
	}
...
uint64_t Memory::patternScan(const char* pattern, const std::string& mask)
{
